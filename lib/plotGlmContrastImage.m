...
%  flatROIs     functional flat map ROI names (default: {})

function [Tth, Vts, Vfs, Tmaxs, Tcnts] = plotGlmContrastImage(contnames, Ts, thParam, clParam, atlasV, isMask, isRtoL, backV, sessionName, rangePlus, rangeMinus, cmap, zidx, flatXY, flatROIs)
    if nargin < 15, flatROIs = {}; end
...
%        T2(T2>-Tth & T2<Tth) = nan;
        ...
        if ~isempty(flatXY)
            ...
            figure; plotNifti3Dflatmap(T2p, atlasV, isMask, flatXY, 10, rangePlus, cmap, [0.1 0.1 0.1], [0 0 0], 'mean', flatROIs);
            title(['GLM contrast (plus) of ' sessionName ' : ' contnames{j}]);

            figure; plotNifti3Dflatmap(T2m, atlasV, isMask, flatXY, 10, rangeMinus, cmap, [0.1 0.1 0.1], [0 0 0], 'mean', flatROIs);
